...
PROJECT_ID = 'information-retrival-ass3'
...
class InvertedIndex:  
    ...
    def read_a_posting_list(self,w,base_dir='', bucket_name=None):
        posting_list = []
        if not w in self.posting_locs:
            return posting_list

        # Retrieve locations for the term
        locs = self.posting_locs[w]
        
        # Print the locations for debugging or logging
        # print(f"Locations for term '{w}': {locs}")
        locs = [loc for loc in locs if loc[0].startswith(self.true_location)]

        # Print the locations for debugging or logging
        # print(f"Locations for term '{w}': {locs}")

        with closing(MultiFileReader(base_dir, bucket_name)) as reader:
            # Assuming `locs` contains tuples of (file_name, offset), and
            # you have a method to read based on these locations.
            b = reader.read(locs, self.df[w] * TUPLE_SIZE)
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i*TUPLE_SIZE:i*TUPLE_SIZE+4], 'big')
                tf = int.from_bytes(b[i*TUPLE_SIZE+4:(i+1)*TUPLE_SIZE], 'big')
